% This script is to test the @edgepot edge potential class designed to handle
% a Continuous Markov Random Field with edge potential updates. It is contained by
% the class pmrf and inference is through particle belief node class pnode using 
...
for l=1:size(E,1)
    ...
    myedgecfg.updatefun = @symedgepotupdate;
...
for i=2:mygraph.N
    ...
    if ~isempty( mygraph.nodes(i).initstate )
        priormargs_bp(i) = [mygraph.nodes(i).initstate ];
    else
         priormargs_bp(i) = particles('states', rand(2,100) );
    end
end
...
rgbobj = rgb;
for i=1:length( E )
    plot( mygraph.edgepots(i).tupdate,'Color',rgbobj.getcol );
end
xlabel('iteration')
