...
% save bearingsensortestdata21.mat sim simcfg Xt

% % Now initiate a filter and run it
% 
load bearingsensortestdata21.mat
...
% sensors 2 and 3 measure one of the targets, sensors 1 and 4 measure the
% other
selsensors = [2,4,1,3];
%selsensors = [2,3];
%selsensors = [1,4];
...
for i=1:length(fschedule.ts)
    ...
    for j=1:length( fschedule.senschedule{i} )
        ...
        aoa =   sensors( fschedule.senschedule{i}(j) ).srbuffer( fschedule.srptrall{i}(j) ).Z.bearing + ...
