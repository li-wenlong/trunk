...
% pcfg.state = [-750 750 40 0]';
% 
% pcfg.stfswitch = [0,20,40];
% pcfg.stfs = {'circmotion','lingaussain','die'};
% 
% % Configure the state transition function
% circfg = stf_circmotioncfg; % Get the config 
% %circfg.statelabels = {'x','y', 'vx','vy'};
% %circfg.state = [0 1500 20 0]';
% circfg.clockwise = 1;
% circfg.deltat = 0.01;
% circfg.centre = [0,0,0]';
% circfg.inittime = 0;
% 
% pcfg.stfcfgs{1} = circfg; % % Subs. the config
% % Second config
...
% 
% 
% pcfg.stfcfgs{2} = lingausscfg; % % Subs. the config
% pcfg.stfcfgs{3} = stf_diecfg;
...
% pcfg = platformcfg;
% pcfg.statelabels = {'x','y','vx','vy'};
% pcfg.state = [1500 1000 0 30]';
% 
% pcfg.stfswitch = [0,40];
% pcfg.stfs = {'circmotion','die'};
% % Configure the state transition function
% circfg = stf_circmotioncfg; % Get the config 
% circfg.clockwise = 0;
% circfg.deltat = 0.01;
% circfg.centre = [1000, 1000, 0]';
% circfg.inittime = 0; %  Subs. the config
% 
% pcfg.stfcfgs{1} = circfg; % % Subs. the config
...
% pcfg = platformcfg;
% pcfg.statelabels = {'x','y','vx','vy'};
% pcfg.state = [1000 -750 -20 0]';
% 
% pcfg.stfswitch = [0,15,40];
% pcfg.stfs = {'circmotion','lingauss','die'};
% 
% % Configure the state transition function
% circfg = stf_circmotioncfg; % Get the config 
% circfg.clockwise = 1;
% circfg.deltat = 0.01;
% circfg.centre = [1000,0,0]';
% circfg.inittime = 0; %  Subs. the config
% 
% pcfg.stfcfgs{1} = circfg; % % Subs. the config
% % Second config
...
filtercfg.veldist = gmm(1',gk( [25^2 0;0 25^2], [ 0 0]'));
% 
