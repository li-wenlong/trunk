...
% 
% 
% %% Configure platform 3 w/ eosensor
% deltat = 1;
% pcfg = platformcfg;
% pcfg.statelabels = {'x','y','vx','vy'};
% pcfg.state = [0 0 0 0]';
...
% sim = msmosim( simcfg );
% sim.run;
% [Xt, numxt] = sim.getmostates([3]');
% 
% save eosensortestdata2.mat sim simcfg Xt

% % % Now initiate a filter and run it
% % 
load eosensortestdata2.mat
...
velstd = 2;
gk1 = gk( [(velstd)^2 0;0 (velstd*8)^2], [ 22 0]');
gk2 = gk( [velstd^2 0;0 (velstd*8)^2], [ -22 0]');
gk3 = gk( [(velstd*8)^2 0;0 velstd^2], [0 22]');
gk4 = gk( [(velstd*8)^2 0;0 velstd^2], [0 -22]');
