...
R = norm(fd);
fd = fd/R;
% ae = ((2*pi/this.T)*speed)*fd;
ae = speed^2/R*fd;
% Take the part of ae that is perpendiculat to v
ae = ae - (ae'*v/speed)*v/speed;
% Compansate for the speed difference
ae = ae + ( (prevspeed-speed)*v/speed)/this.deltat;


a = dcm(eang)*ae; % Find a in body fixed
%%%%%%%%%%%%%%%%%%%%%%%%%%%
this.acceleration = a;    
