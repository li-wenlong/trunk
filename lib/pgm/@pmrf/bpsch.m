function varargout = bpsch( this, varargin )
...
for i=finum:linum
    ...
    mypat = this.mschedule.getithentry(i);
    for k=1:size( mypat )
        e = mypat(k,:);
        if DEBUG_VERBOSE && DEBUG_PMRF
        ...
        message2pass = tnode.compnbpmessagewav( e(2) ); % Compute the outgoint message
    ...
    myupd = this.uschedule.getithentry(i);
    for k=1:length( myupd )
        % Update the node states if scheduled
        v = myupd(k);
        unode = this.nodes( v );
        if DEBUG_VERBOSE && DEBUG_PMRF
            disp(sprintf('Updating the state of node %d',v ));
        end
        unode.updateparstatewav;
        this.nodes( v ) = unode;
    end
