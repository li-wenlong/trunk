function [n, m] = CalculateMessage( n )
...
for checkFor = 1:length(n.neighbours)
    ...
    if ~ismember( n.neighbours(checkFor), sentMessages )
        
        % if we have received messages from all neighbours except this one
        if sum( ismember( receivedMessages, setdiff( n.neighbours, n.neighbours(checkFor)) )) == length(n.neighbours) - 1
