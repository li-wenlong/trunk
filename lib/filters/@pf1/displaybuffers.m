...
if ~isempty(predintensity) && axisHandles(1) ~= 0
    ...
    predintensity.draw( 'axis', axisHandles(1) ,'options', plotOpts{1} ,'postcommands', postcommands, 'precommands', precommands, 'dims', dims );
    axes( axisHandles(1) );
  %  ax_ = axis;
...
if ~isempty( postintensity ) && axisHandles(3) ~= 0
    ...
    postintensity.draw('axis', axisHandles(3), 'options', plotOpts{1} , 'postcommands', postcommands ,  'precommands', precommands, 'dims', dims );
    axes( axisHandles(3) );
   % ax_ = axis;
   % ax_(1:4) = [-6000,6000,-6000,6000];
   % axis(ax_)
