   ...
   if ~isempty( findincells( fieldnames(this), {'veldist'} ) ) && ~isgen
    ...
    if ~isgen
        if isa( this.speedminmax, 'cell' )
            ...
            mtheta = [rand(1, numpartnewborn)*2*pi];
            mspeed = [rand(1, numpartnewborn )*abs(this.speedminmax(2)-this.speedminmax(1) ) + ( this.speedminmax(1) )]; 
            nbstates = [locstates; mspeed.*cos(mtheta); mspeed.*sin(mtheta)];
            isgen = 1;
        end
