...
for i = 1:numcomps
   ...
   % Give a go with non whitened particles: To reverse remove the comment
   % outed line below and comment out the other
   % kdes(i) = kde( Wp*pc , bwselection, wc,kernelType  ); % Weights are scaled at constructor
   
   % kdes(i) = kde( pc , bwselection, wc,kernelType  ); % Weights are scaled at constructor
