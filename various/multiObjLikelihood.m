function L=multiObjLikelihood(z,x,H,R)
    ...
    for i=1:gamma
        theta=(i/gamma)*pi-(pi/2); %Use this equation for the side of an ellipse above the major axis,
                                   %otherwise theta=(i/gamma)*pi+(pi/2);
        beta=angle*pi/180;
        x_pt=[1 0]*H*m+major_axis*cos(theta)*cos(beta)-minor_axis*sin(theta)*sin(beta);
        y_pt=[0 1]*H*m+major_axis*cos(theta)*sin(beta)+minor_axis*sin(theta)*cos(beta);
        ellipsePt{i}=[x_pt y_pt]';
    end
