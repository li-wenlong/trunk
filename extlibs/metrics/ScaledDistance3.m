function [dist varargout]= ScaledDistance3(X, CvX, Y, CvY, CutOff, Power, Epsilon)
...
[Matching, cost]= Hungarian(DistanceMatrix);
if sum(Matching(:)) == 0
    cost = nan;
end
...
DistanceMatrix = zeros(NumX*NumY, 1);
for count = 1:NumX*NumY
    DistanceMatrix(count) = MVNKLDivergence( ... 
        XX(:,count), CvXX(:,:,count), YY(:,count), CvYY(:,:,count));
end
